...
<#
.SYNOPSIS
    Pester offline unit tests for Get-Eventtypes.ps1
...
BeforeAll {
    $ScriptPath = Join-Path $PSScriptRoot '..' 'Get-Eventtypes.ps1'
    $FixturePath = Join-Path $PSScriptRoot 'fixtures'
}

Describe 'Get-Eventtypes.ps1' -Tag 'Offline', 'Notification' {
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'list_event_types_200.json') -Raw | ConvertFrom-Json
        }
