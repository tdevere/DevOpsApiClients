...
"""
Offline unit tests for list_diagnostic_logs.py
...
SCRIPT = str(Path(__file__).resolve().parents[1] / "list_diagnostic_logs.py")
...
class TestListDiagnosticLogsEnvValidation:
    ...
    @pytest.mark.offline
    @pytest.mark.notification
    def test_missing_entry_id_exits(self):
        result = _run_script(
            env_override={"ENTRY_ID": None},
            expect_fail=True,
        )
        assert result.returncode != 0
        assert "ENTRY_ID" in result.stderr or "ENTRY_ID" in result.stdout


class TestListDiagnosticLogsURLAndAuth:
    ...
    def test_correct_url_and_auth_header(self):
        ...
        expected_url = "https://{service}dev.azure.com/testorg/_apis/notification/diagnosticlogs/test-value-source/entries/test-value-entry_id?api-version=7.2"

        fixture = json.loads((FIXTURES / "list_diagnostic_logs_200.json").read_text())
...
class TestListDiagnosticLogsHTTPErrors:
    ...
    def test_401_unauthorized(self):
        url = "https://{service}dev.azure.com/testorg/_apis/notification/diagnosticlogs/test-value-source/entries/test-value-entry_id?api-version=7.2"
