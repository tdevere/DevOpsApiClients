...
<#
.SYNOPSIS
    Pester offline unit tests for Get-Favorite.ps1
...
BeforeAll {
    $ScriptPath = Join-Path $PSScriptRoot '..' 'Get-Favorite.ps1'
    $FixturePath = Join-Path $PSScriptRoot 'fixtures'
}

Describe 'Get-Favorite.ps1' -Tag 'Offline', 'Favorite' {
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'get_favorites_200.json') -Raw | ConvertFrom-Json
        }
