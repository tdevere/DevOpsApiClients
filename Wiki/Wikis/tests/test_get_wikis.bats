...
# ============================================================================
# Offline unit tests for get_wikis.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/wiki"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/get_wikis_200.json"
        exit 0
    fi
done
...
@test "get_wikis.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/get_wikis.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "get_wikis.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "get_wikis.sh: exits with error when PROJECT_ID is unset" {
    unset PROJECT_ID
    run bash "$SCRIPT_DIR/get_wikis.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"PROJECT_ID"* ]]
...
@test "get_wikis.sh: exits with error when WIKI_IDENTIFIER is unset" {
...
@test "get_wikis.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/get_wikis.sh"
    [ "$status" -eq 0 ]
}

@test "get_wikis.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/get_wikis.sh"
    [ "$status" -eq 0 ]
