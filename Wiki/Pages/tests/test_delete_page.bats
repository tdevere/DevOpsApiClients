...
@test "delete_page.sh: exits with error when PROJECT_ID is unset" {
    unset PROJECT_ID
    run bash "$SCRIPT_DIR/delete_page.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"PROJECT_ID"* ]]
...
@test "delete_page.sh: exits with error when WIKI_IDENTIFIER is unset" {
...
@test "delete_page.sh: exits with error when WIKI_PAGE_PATH is unset" {
