...
# ============================================================================
# Offline unit tests for get_page.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/wiki"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/get_page_200.json"
        exit 0
    fi
done
...
@test "get_page.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/get_page.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "get_page.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "get_page.sh: exits with error when PROJECT_ID is unset" {
    unset PROJECT_ID
    run bash "$SCRIPT_DIR/get_page.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"PROJECT_ID"* ]]
...
@test "get_page.sh: exits with error when WIKI_IDENTIFIER is unset" {
...
@test "get_page.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/get_page.sh"
    [ "$status" -eq 0 ]
}

@test "get_page.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/get_page.sh"
    [ "$status" -eq 0 ]
