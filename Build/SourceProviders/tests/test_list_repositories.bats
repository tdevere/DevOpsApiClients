...
@test "list_repositories.sh: exits with error when PROJECT_ID is unset" {
    unset PROJECT_ID
    run bash "$SCRIPT_DIR/list_repositories.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"PROJECT_ID"* ]]
...
@test "list_repositories.sh: exits with error when PROVIDER_NAME is unset" {
    unset PROVIDER_NAME
    run bash "$SCRIPT_DIR/list_repositories.sh"
    [ "$status" -ne 0 ]
...
@test "list_repositories.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/list_repositories.sh"
    [ "$status" -eq 0 ]
}
