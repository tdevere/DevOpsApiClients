...
# ============================================================================
# Offline unit tests for run_builds.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/build"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/run_builds_200.json"
        exit 0
    fi
done
...
@test "run_builds.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/run_builds.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "run_builds.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "run_builds.sh: exits with error when PROJECT_ID is unset" {
    unset PROJECT_ID
    run bash "$SCRIPT_DIR/run_builds.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"PROJECT_ID"* ]]
...
@test "run_builds.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/run_builds.sh"
    [ "$status" -eq 0 ]
}

@test "run_builds.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/run_builds.sh"
    [ "$status" -eq 0 ]
