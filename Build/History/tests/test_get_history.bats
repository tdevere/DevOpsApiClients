...
# ============================================================================
# Offline unit tests for get_history.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/build"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/get_history_200.json"
        exit 0
    fi
done
...
@test "get_history.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/get_history.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "get_history.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "get_history.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/get_history.sh"
    [ "$status" -eq 0 ]
}

@test "get_history.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/get_history.sh"
    [ "$status" -eq 0 ]
