...
# ============================================================================
# Offline unit tests for get_health.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/status"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/get_health_200.json"
        exit 0
    fi
done
...
@test "get_health.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/get_health.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "get_health.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "get_health.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/get_health.sh"
    [ "$status" -eq 0 ]
}

@test "get_health.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/get_health.sh"
    [ "$status" -eq 0 ]
