...
<#
.SYNOPSIS
    Pester offline unit tests for Get-Orgenablement.ps1
...
BeforeAll {
    $ScriptPath = Join-Path $PSScriptRoot '..' 'Get-Orgenablement.ps1'
    $FixturePath = Join-Path $PSScriptRoot 'fixtures'
}

Describe 'Get-Orgenablement.ps1' -Tag 'Offline', 'AdvancedSecurity' {
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'get_org_enablement_200.json') -Raw | ConvertFrom-Json
        }
