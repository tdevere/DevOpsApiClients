...
# ============================================================================
# Offline unit tests for delete_folders.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/release"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/delete_folders_200.json"
        exit 0
    fi
done
...
@test "delete_folders.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
...
@test "delete_folders.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "delete_folders.sh: exits with error when PROJECT_ID is unset" {
    unset PROJECT_ID
    run bash "$SCRIPT_DIR/delete_folders.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"PROJECT_ID"* ]]
...
@test "delete_folders.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/delete_folders.sh"
    [ "$status" -eq 0 ]
