...
# ============================================================================
# Offline unit tests for create_feed.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/packaging"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/create_feed_200.json"
        exit 0
    fi
done
...
@test "create_feed.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/create_feed.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "create_feed.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "create_feed.sh: exits with error when PROJECT_ID is unset" {
    unset PROJECT_ID
    run bash "$SCRIPT_DIR/create_feed.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"PROJECT_ID"* ]]
...
@test "create_feed.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/create_feed.sh"
    [ "$status" -eq 0 ]
}

@test "create_feed.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/create_feed.sh"
    [ "$status" -eq 0 ]
