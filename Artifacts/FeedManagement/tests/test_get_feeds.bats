...
# ============================================================================
# Offline unit tests for get_feeds.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/packaging"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/get_feeds_200.json"
        exit 0
    fi
done
...
@test "get_feeds.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/get_feeds.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "get_feeds.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "get_feeds.sh: exits with error when PROJECT_ID is unset" {
    unset PROJECT_ID
    run bash "$SCRIPT_DIR/get_feeds.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"PROJECT_ID"* ]]
...
@test "get_feeds.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/get_feeds.sh"
    [ "$status" -eq 0 ]
}

@test "get_feeds.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/get_feeds.sh"
    [ "$status" -eq 0 ]
