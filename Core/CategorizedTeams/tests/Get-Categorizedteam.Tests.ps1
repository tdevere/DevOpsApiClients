...
<#
.SYNOPSIS
    Pester offline unit tests for Get-Categorizedteam.ps1
...
BeforeAll {
    $ScriptPath = Join-Path $PSScriptRoot '..' 'Get-Categorizedteam.ps1'
    $FixturePath = Join-Path $PSScriptRoot 'fixtures'
}

Describe 'Get-Categorizedteam.ps1' -Tag 'Offline', 'Core' {

    Context 'Input validation' {

        It 'Throws when Organisation is empty' {
            {
                & $ScriptPath -Organization '' -ProjectId 'testvalue' -Pat 'fakepat'
        ...
        It 'Throws when Pat is empty' {
            {
                & $ScriptPath -Organization 'testorg' -ProjectId 'testvalue' -Pat ''
            } | Should -Throw '*is required*'
        }
    }
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'get_categorized_teams_200.json') -Raw | ConvertFrom-Json
        }

        It 'Calls the correct URL' {
            Mock Invoke-RestMethod { return $fixture } -Verifiable

            & $ScriptPath -Organization 'testorg' -ProjectId 'testvalue' -Pat 'fakepat' | Out-Null

            Should -InvokeVerifiable
            Should -Invoke Invoke-RestMethod -Times 1 -Exactly -ParameterFilter {
                $Uri -eq "https://dev.azure.com/testorg/_apis/projects/testvalue/categorizedteams/?api-version=7.2" -and
                $Method -eq 'Get'
            }
        }
    ...
    Context 'HTTP error handling' {
        ...
        It 'Throws on 404 (not found)' {
            ...
            { & $ScriptPath -Organization 'testorg' -ProjectId 'testvalue' -Pat 'fakepat' } | Should -Throw
        }
    }
}
