...
<#
.SYNOPSIS
    Pester offline unit tests for New-Project.ps1
...
Describe 'New-Project.ps1' -Tag 'Offline', 'Core' {

    Context 'Input validation' {

        It 'Throws when Organisation is empty' {
            {
                & $ScriptPath -Organization '' -Capabilities 'testvalue' -DefaultTeam 'testvalue' -Pat 'fakepat'
            } | Should -Throw '*is required*'
        }

        It 'Throws when Pat is empty' {
            {
                & $ScriptPath -Organization 'testorg' -Capabilities 'testvalue' -DefaultTeam 'testvalue' -Pat ''
            } | Should -Throw '*is required*'
        }
    }
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'create_projects_200.json') -Raw | ConvertFrom-Json
        }

        It 'Calls the correct URL' {
            Mock Invoke-RestMethod { return $fixture } -Verifiable

            & $ScriptPath -Organization 'testorg' -Capabilities 'testvalue' -DefaultTeam 'testvalue' -Pat 'fakepat' | Out-Null

            Should -InvokeVerifiable
            Should -Invoke Invoke-RestMethod -Times 1 -Exactly -ParameterFilter {
                $Uri -eq "https://dev.azure.com/testorg/_apis/projects?api-version=7.2" -and
                $Method -eq 'Post'
            }
        }
    ...
    Context 'HTTP error handling' {
        ...
        It 'Throws on 404 (not found)' {
            ...
            { & $ScriptPath -Organization 'testorg' -Capabilities 'testvalue' -DefaultTeam 'testvalue' -Pat 'fakepat' } | Should -Throw
        }
    }
}
