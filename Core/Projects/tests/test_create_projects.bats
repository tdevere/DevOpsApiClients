...
# ============================================================================
# Offline unit tests for create_projects.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/projects"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/create_projects_200.json"
        exit 0
    fi
done
...
@test "create_projects.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/create_projects.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "create_projects.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "create_projects.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/create_projects.sh"
    [ "$status" -eq 0 ]
}

@test "create_projects.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/create_projects.sh"
    [ "$status" -eq 0 ]
