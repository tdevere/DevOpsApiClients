#!/usr/bin/env bats
# ============================================================================
# Offline unit tests for get_project.sh
#
...
@test "get_project.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/get_project.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "get_project.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "get_project.sh: exits with error when PROJECT_ID is unset" {
    unset PROJECT_ID
    run bash "$SCRIPT_DIR/get_project.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"PROJECT_ID"* ]]
...
@test "get_project.sh: calls correct URL with project ID in path" {
    run bash "$SCRIPT_DIR/get_project.sh"
    [ "$status" -eq 0 ]
...
@test "get_project.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/get_project.sh"
    [ "$status" -eq 0 ]
...
@test "get_project.sh: outputs project details from fixture" {
    run bash "$SCRIPT_DIR/get_project.sh"
    [ "$status" -eq 0 ]
