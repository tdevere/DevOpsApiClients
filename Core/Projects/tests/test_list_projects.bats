#!/usr/bin/env bats
# ============================================================================
# Offline unit tests for list_projects.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/projects?"* ]]; then
        cat "${FIXTURES_DIR}/list_projects_200.json"
        exit 0
    fi
done
...
@test "list_projects.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/list_projects.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "list_projects.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "list_projects.sh: calls correct URL with api-version" {
    run bash "$SCRIPT_DIR/list_projects.sh"
    [ "$status" -eq 0 ]

    # Verify curl was called with the correct URL
    curl_args=$(cat "${BATS_TMPDIR}/curl_args.log")
    [[ "$curl_args" == *"https://dev.azure.com/testorg/_apis/projects?api-version=7.2-preview.4"* ]]
}

@test "list_projects.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/list_projects.sh"
    [ "$status" -eq 0 ]
...
@test "list_projects.sh: outputs valid JSON response" {
    run bash "$SCRIPT_DIR/list_projects.sh"
    [ "$status" -eq 0 ]

    # Output should contain project data from fixture
    [[ "$output" == *"count"* ]]
    [[ "$output" == *"ProjectAlpha"* ]]
}
