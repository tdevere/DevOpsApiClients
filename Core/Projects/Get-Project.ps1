...
.DESCRIPTION
    Calls  GET {org}/_apis/projects/{project_id}?api-version=7.2
...
#--- Version guard ----------------------------------------------------------
if ($response.PSObject.Properties.Name -notcontains 'capabilities') {
    Write-Warning "Unexpected response shape — missing 'capabilities'. The server may not support api-version $ApiVersion."
}
if ($response.PSObject.Properties.Name -notcontains 'defaultTeam') {
    Write-Warning "Unexpected response shape — missing 'defaultTeam'. The server may not support api-version $ApiVersion."
}
