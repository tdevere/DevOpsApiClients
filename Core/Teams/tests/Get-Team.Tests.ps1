...
<#
.SYNOPSIS
    Pester offline unit tests for Get-Team.ps1
...
BeforeAll {
    $ScriptPath = Join-Path $PSScriptRoot '..' 'Get-Team.ps1'
    $FixturePath = Join-Path $PSScriptRoot 'fixtures'
}

Describe 'Get-Team.ps1' -Tag 'Offline', 'Core' {
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'get_teams_200.json') -Raw | ConvertFrom-Json
        }
