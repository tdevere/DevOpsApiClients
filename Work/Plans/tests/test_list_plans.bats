...
# ============================================================================
# Offline unit tests for list_plans.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/work"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/list_plans_200.json"
        exit 0
    fi
done
...
@test "list_plans.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/list_plans.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "list_plans.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "list_plans.sh: exits with error when PROJECT_ID is unset" {
    unset PROJECT_ID
    run bash "$SCRIPT_DIR/list_plans.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"PROJECT_ID"* ]]
...
@test "list_plans.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/list_plans.sh"
    [ "$status" -eq 0 ]
}

@test "list_plans.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/list_plans.sh"
    [ "$status" -eq 0 ]
