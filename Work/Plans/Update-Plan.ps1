...
.DESCRIPTION
    Calls  PUT {org}/_apis/work/plans/{id}?api-version=7.2
    Uses Basic Auth with a PAT stored in $env:AZURE_DEVOPS_PAT.

.LINK
    https://learn.microsoft.com/en-us/rest/api/azure/devops/work/plans/update?view=azure-devops-rest-7.2
#>
...
#--- Call the API -----------------------------------------------------------
$uri = New-AdoUrl -Organization $Organization -Path "_apis/work/plans/$Id" -ApiVersion $ApiVersion -Project $ProjectId
...
if ($response.PSObject.Properties.Name -notcontains 'createdByIdentity') {
    Write-Warning "Unexpected response shape — missing 'createdByIdentity'. The server may not support api-version $ApiVersion."
}

#--- Output -----------------------------------------------------------------
Write-Host "Plans: $($response.name)"
