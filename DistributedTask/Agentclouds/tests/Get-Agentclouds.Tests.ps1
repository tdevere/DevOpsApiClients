...
<#
.SYNOPSIS
    Pester offline unit tests for Get-Agentclouds.ps1
...
BeforeAll {
    $ScriptPath = Join-Path $PSScriptRoot '..' 'Get-Agentclouds.ps1'
    $FixturePath = Join-Path $PSScriptRoot 'fixtures'
}

Describe 'Get-Agentclouds.ps1' -Tag 'Offline', 'DistributedTask' {
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'list_agentclouds_200.json') -Raw | ConvertFrom-Json
        }
