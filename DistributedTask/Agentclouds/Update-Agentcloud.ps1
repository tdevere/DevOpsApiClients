# AUTO-GENERATED by _generator — do not edit by hand.
# Source of truth: _generator/definitions/
<#
.SYNOPSIS
    Update for Agentclouds

.DESCRIPTION
    Calls  PATCH {org}/_apis/distributedtask/agentclouds/{agent_cloud_id}?api-version=7.2
    Uses Basic Auth with a PAT stored in $env:AZURE_DEVOPS_PAT.

.LINK
    https://learn.microsoft.com/en-us/rest/api/azure/devops/distributed-task/agentclouds/update?view=azure-devops-rest-7.2
#>

[CmdletBinding()]
param (
    [Parameter(Mandatory)]
    [string]$AcquireAgentEndpoint,

    [Parameter(Mandatory)]
    [string]$AcquisitionTimeout,

    [Parameter(Mandatory)]
    [string]$GetAccountParallelismEndpoint,

    [Parameter(Mandatory)]
    [string]$GetAgentDefinitionEndpoint,

    [Parameter(Mandatory)]
    [string]$GetAgentRequestStatusEndpoint,

    [Parameter()]
    [string]$Organization = $env:AZURE_DEVOPS_ORG,

    [Parameter()]
    [string]$AgentCloudId = $env:AGENT_CLOUD_ID,

    [Parameter()]
    [string]$Pat = $env:AZURE_DEVOPS_PAT
)

Set-StrictMode -Version Latest
$ErrorActionPreference = 'Stop'

#--- Shared helpers ---------------------------------------------------------
$_sharedDir = Join-Path (Join-Path (Split-Path $PSScriptRoot -Parent) '..') '_shared'
. (Join-Path $_sharedDir 'AdoAuth.ps1')
. (Join-Path $_sharedDir 'AdoHttp.ps1')

#--- Validate inputs --------------------------------------------------------
Assert-AdoEnv -Name 'Organisation' -Value $Organization
Assert-AdoEnv -Name 'AgentCloudId' -Value $AgentCloudId
Assert-AdoEnv -Name 'PAT' -Value $Pat

#--- API version ------------------------------------------------------------
$ApiVersion = '7.2'

#--- Build auth header ------------------------------------------------------
$headers = New-AdoAuthHeader -Pat $Pat

#--- Build request body -----------------------------------------------------
$body = @{
    acquireAgentEndpoint = $AcquireAgentEndpoint
    acquisitionTimeout = $AcquisitionTimeout
    agentCloudId = $AgentCloudId
    getAccountParallelismEndpoint = $GetAccountParallelismEndpoint
    getAgentDefinitionEndpoint = $GetAgentDefinitionEndpoint
    getAgentRequestStatusEndpoint = $GetAgentRequestStatusEndpoint
} | ConvertTo-Json -Depth 3

#--- Call the API -----------------------------------------------------------
$uri = New-AdoUrl -Organization $Organization -Path "_apis/distributedtask/agentclouds/$AgentCloudId" -ApiVersion $ApiVersion

Write-Verbose "PATCH $uri"

$response = Invoke-RestMethod -Uri $uri -Method Patch -Headers $headers -Body $body -ContentType 'application/json'

#--- Version guard ----------------------------------------------------------
if ($response.PSObject.Properties.Name -notcontains 'id') {
    Write-Warning "Unexpected response shape — missing 'id'. The server may not support api-version $ApiVersion."
}
if ($response.PSObject.Properties.Name -notcontains 'name') {
    Write-Warning "Unexpected response shape — missing 'name'. The server may not support api-version $ApiVersion."
}
if ($response.PSObject.Properties.Name -notcontains 'acquireAgentEndpoint') {
    Write-Warning "Unexpected response shape — missing 'acquireAgentEndpoint'. The server may not support api-version $ApiVersion."
}

#--- Output -----------------------------------------------------------------
Write-Host "Agentclouds ID: $($response.id)"
$response | ConvertTo-Json -Depth 5
