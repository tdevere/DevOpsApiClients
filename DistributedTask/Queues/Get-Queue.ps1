...
.DESCRIPTION
    Calls  GET {org}/_apis/distributedtask/queues/{queue_id}?api-version=7.2
    Uses Basic Auth with a PAT stored in $env:AZURE_DEVOPS_PAT.

.LINK
    https://learn.microsoft.com/en-us/rest/api/azure/devops/distributed-task/queues/get?view=azure-devops-rest-7.2
#>
...
Assert-AdoEnv -Name 'QueueId' -Value $QueueId
...
#--- Call the API -----------------------------------------------------------
$uri = New-AdoUrl -Organization $Organization -Path "_apis/distributedtask/queues/$QueueId" -ApiVersion $ApiVersion -Project $ProjectId
