...
<#
.SYNOPSIS
    Pester offline unit tests for Add-Queue.ps1
...
BeforeAll {
    $ScriptPath = Join-Path $PSScriptRoot '..' 'Add-Queue.ps1'
    $FixturePath = Join-Path $PSScriptRoot 'fixtures'
    $ProjectGuid = 'a1b2c3d4-e5f6-7890-abcd-ef1234567890'
}

Describe 'Add-Queue.ps1' -Tag 'Offline', 'DistributedTask' {
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'create_queues_200.json') -Raw | ConvertFrom-Json
        }
