...
# ============================================================================
# Offline unit tests for list_pools.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/distributedtask"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/list_pools_200.json"
        exit 0
    fi
done
...
@test "list_pools.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/list_pools.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "list_pools.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "list_pools.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/list_pools.sh"
    [ "$status" -eq 0 ]
}

@test "list_pools.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/list_pools.sh"
    [ "$status" -eq 0 ]
