...
.DESCRIPTION
    Calls  GET {org}/_apis/distributedtask/environments/{environment_id}?api-version=7.2-preview.1
    Uses Basic Auth with a PAT stored in $env:AZURE_DEVOPS_PAT.

.LINK
    https://learn.microsoft.com/en-us/rest/api/azure/devops/distributedtask/environments/get?view=azure-devops-rest-7.2
#>
...
#--- Call the API -----------------------------------------------------------
$uri = New-AdoUrl -Organization $Organization -Path "_apis/distributedtask/environments/$EnvironmentId" -ApiVersion $ApiVersion -Project $ProjectId
...
#--- Output -----------------------------------------------------------------
Write-Host "Environment: $($response.name) (ID: $($response.id))"
