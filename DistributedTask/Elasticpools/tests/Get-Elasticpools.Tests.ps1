...
<#
.SYNOPSIS
    Pester offline unit tests for Get-Elasticpools.ps1
...
BeforeAll {
    $ScriptPath = Join-Path $PSScriptRoot '..' 'Get-Elasticpools.ps1'
    $FixturePath = Join-Path $PSScriptRoot 'fixtures'
}

Describe 'Get-Elasticpools.ps1' -Tag 'Offline', 'DistributedTask' {
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'list_elasticpools_200.json') -Raw | ConvertFrom-Json
        }
