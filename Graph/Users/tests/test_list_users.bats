...
# ============================================================================
# Offline unit tests for list_users.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/graph"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/list_users_200.json"
        exit 0
    fi
done
...
@test "list_users.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/list_users.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "list_users.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "list_users.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/list_users.sh"
    [ "$status" -eq 0 ]
}

@test "list_users.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/list_users.sh"
    [ "$status" -eq 0 ]
