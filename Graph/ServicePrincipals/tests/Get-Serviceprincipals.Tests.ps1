...
<#
.SYNOPSIS
    Pester offline unit tests for Get-Serviceprincipals.ps1
...
BeforeAll {
    $ScriptPath = Join-Path $PSScriptRoot '..' 'Get-Serviceprincipals.ps1'
    $FixturePath = Join-Path $PSScriptRoot 'fixtures'
}

Describe 'Get-Serviceprincipals.ps1' -Tag 'Offline', 'Graph' {
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'list_service_principals_200.json') -Raw | ConvertFrom-Json
        }
