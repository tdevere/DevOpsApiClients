...
<#
.SYNOPSIS
    Pester offline unit tests for Get-Groups.ps1
...
BeforeAll {
    $ScriptPath = Join-Path $PSScriptRoot '..' 'Get-Groups.ps1'
    $FixturePath = Join-Path $PSScriptRoot 'fixtures'
}

Describe 'Get-Groups.ps1' -Tag 'Offline', 'Graph' {
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'list_groups_200.json') -Raw | ConvertFrom-Json
        }
