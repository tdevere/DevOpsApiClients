...
# ============================================================================
# Offline unit tests for list_groups.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/graph"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/list_groups_200.json"
        exit 0
    fi
done
...
@test "list_groups.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/list_groups.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "list_groups.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "list_groups.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/list_groups.sh"
    [ "$status" -eq 0 ]
}

@test "list_groups.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/list_groups.sh"
    [ "$status" -eq 0 ]
