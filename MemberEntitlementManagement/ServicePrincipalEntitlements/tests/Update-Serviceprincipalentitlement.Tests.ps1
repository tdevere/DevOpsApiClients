...
<#
.SYNOPSIS
    Pester offline unit tests for Update-Serviceprincipalentitlement.ps1
...
BeforeAll {
    $ScriptPath = Join-Path $PSScriptRoot '..' 'Update-Serviceprincipalentitlement.ps1'
    $FixturePath = Join-Path $PSScriptRoot 'fixtures'
}

Describe 'Update-Serviceprincipalentitlement.ps1' -Tag 'Offline', 'MemberEntitlementManagement' {
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'update_service_principal_entitlements_200.json') -Raw | ConvertFrom-Json
        }
