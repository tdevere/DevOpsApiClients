...
<#
.SYNOPSIS
    Pester offline unit tests for Get-Groupentitlements.ps1
...
BeforeAll {
    $ScriptPath = Join-Path $PSScriptRoot '..' 'Get-Groupentitlements.ps1'
    $FixturePath = Join-Path $PSScriptRoot 'fixtures'
}

Describe 'Get-Groupentitlements.ps1' -Tag 'Offline', 'MemberEntitlementManagement' {
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'list_group_entitlements_200.json') -Raw | ConvertFrom-Json
        }
