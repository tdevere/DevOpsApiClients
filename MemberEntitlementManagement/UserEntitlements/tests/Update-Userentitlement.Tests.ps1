...
<#
.SYNOPSIS
    Pester offline unit tests for Update-Userentitlement.ps1
...
BeforeAll {
    $ScriptPath = Join-Path $PSScriptRoot '..' 'Update-Userentitlement.ps1'
    $FixturePath = Join-Path $PSScriptRoot 'fixtures'
}

Describe 'Update-Userentitlement.ps1' -Tag 'Offline', 'MemberEntitlementManagement' {
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'update_user_entitlements_200.json') -Raw | ConvertFrom-Json
        }
