...
<#
.SYNOPSIS
    Pester offline unit tests for Invoke-Userentitlements.ps1
...
BeforeAll {
    $ScriptPath = Join-Path $PSScriptRoot '..' 'Invoke-Userentitlements.ps1'
    $FixturePath = Join-Path $PSScriptRoot 'fixtures'
}

Describe 'Invoke-Userentitlements.ps1' -Tag 'Offline', 'MemberEntitlementManagement' {
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'query_user_entitlements_200.json') -Raw | ConvertFrom-Json
        }
