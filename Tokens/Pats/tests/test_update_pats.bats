...
# ============================================================================
# Offline unit tests for update_pats.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/tokens"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/update_pats_200.json"
        exit 0
    fi
done
...
@test "update_pats.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/update_pats.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "update_pats.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "update_pats.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/update_pats.sh"
    [ "$status" -eq 0 ]
}

@test "update_pats.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/update_pats.sh"
    [ "$status" -eq 0 ]
