...
# ============================================================================
# Offline unit tests for create_pats.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/tokens"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/create_pats_200.json"
        exit 0
    fi
done
...
@test "create_pats.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/create_pats.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "create_pats.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "create_pats.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/create_pats.sh"
    [ "$status" -eq 0 ]
}

@test "create_pats.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/create_pats.sh"
    [ "$status" -eq 0 ]
