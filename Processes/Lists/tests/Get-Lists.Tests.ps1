...
<#
.SYNOPSIS
    Pester offline unit tests for Get-Lists.ps1
...
BeforeAll {
    $ScriptPath = Join-Path $PSScriptRoot '..' 'Get-Lists.ps1'
    $FixturePath = Join-Path $PSScriptRoot 'fixtures'
}

Describe 'Get-Lists.ps1' -Tag 'Offline', 'Processes' {
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'list_lists_200.json') -Raw | ConvertFrom-Json
        }
