...
# ============================================================================
# Offline unit tests for create_lists.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/work"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/create_lists_200.json"
        exit 0
    fi
done
...
@test "create_lists.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/create_lists.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "create_lists.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "create_lists.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/create_lists.sh"
    [ "$status" -eq 0 ]
}

@test "create_lists.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/create_lists.sh"
    [ "$status" -eq 0 ]
