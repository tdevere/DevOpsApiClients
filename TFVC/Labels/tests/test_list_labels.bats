...
# ============================================================================
# Offline unit tests for list_labels.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/tfvc"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/list_labels_200.json"
        exit 0
    fi
done
...
@test "list_labels.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/list_labels.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "list_labels.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "list_labels.sh: exits with error when PROJECT_ID is unset" {
    unset PROJECT_ID
    run bash "$SCRIPT_DIR/list_labels.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"PROJECT_ID"* ]]
...
@test "list_labels.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/list_labels.sh"
    [ "$status" -eq 0 ]
}

@test "list_labels.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/list_labels.sh"
    [ "$status" -eq 0 ]
