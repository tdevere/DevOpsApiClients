...
.DESCRIPTION
    Calls  GET {org}/_apis/hooks/consumers/{consumer_id}?api-version=7.2
    Uses Basic Auth with a PAT stored in $env:AZURE_DEVOPS_PAT.

.LINK
    https://learn.microsoft.com/en-us/rest/api/azure/devops/hooks/consumers/get?view=azure-devops-rest-7.2
#>
...
param (
    ...
    [Parameter()]
    [string]$ConsumerId = $env:CONSUMER_ID,
...
Assert-AdoEnv -Name 'ConsumerId' -Value $ConsumerId
...
#--- Call the API -----------------------------------------------------------
$uri = New-AdoUrl -Organization $Organization -Path "_apis/hooks/consumers/$ConsumerId" -ApiVersion $ApiVersion
...
#--- Output -----------------------------------------------------------------
Write-Host "Consumers ID: $($response.id)"
