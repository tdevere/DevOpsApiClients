...
<#
.SYNOPSIS
    Pester offline unit tests for Get-Permissionsreport.ps1
...
BeforeAll {
    $ScriptPath = Join-Path $PSScriptRoot '..' 'Get-Permissionsreport.ps1'
    $FixturePath = Join-Path $PSScriptRoot 'fixtures'
}

Describe 'Get-Permissionsreport.ps1' -Tag 'Offline', 'PermissionsReport' {
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'list_permissions_report_200.json') -Raw | ConvertFrom-Json
        }
