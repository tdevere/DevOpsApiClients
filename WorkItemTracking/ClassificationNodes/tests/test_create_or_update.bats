...
# ============================================================================
# Offline unit tests for create_or_update.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/wit"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/create_or_update_200.json"
        exit 0
    fi
done
...
@test "create_or_update.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
...
@test "create_or_update.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "create_or_update.sh: exits with error when PROJECT_ID is unset" {
    unset PROJECT_ID
    run bash "$SCRIPT_DIR/create_or_update.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"PROJECT_ID"* ]]
...
@test "create_or_update.sh: exits with error when STRUCTURE_GROUP is unset" {
...
@test "create_or_update.sh: exits with error when CLASSIFICATION_PATH is unset" {
...
@test "create_or_update.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/create_or_update.sh"
    [ "$status" -eq 0 ]
