...
.DESCRIPTION
    Calls  GET {org}/_apis/wit/workItems/{id}/revisions/{revision_number}?api-version=7.2
    Uses Basic Auth with a PAT stored in $env:AZURE_DEVOPS_PAT.

.LINK
    https://learn.microsoft.com/en-us/rest/api/azure/devops/wit/revisions/get?view=azure-devops-rest-7.2
#>
...
param (
    ...
    [Parameter()]
    [string]$RevisionNumber = $env:REVISION_NUMBER,
...
Assert-AdoEnv -Name 'RevisionNumber' -Value $RevisionNumber
...
#--- Call the API -----------------------------------------------------------
$uri = New-AdoUrl -Organization $Organization -Path "_apis/wit/workItems/$Id/revisions/$RevisionNumber" -ApiVersion $ApiVersion -Project $ProjectId
...
#--- Output -----------------------------------------------------------------
Write-Host "Revisions ID: $($response.id)"
