...
.DESCRIPTION
    Calls  PATCH {org}/_apis/wit/queries/{query}?api-version=7.2
    Uses Basic Auth with a PAT stored in $env:AZURE_DEVOPS_PAT.

.LINK
    https://learn.microsoft.com/en-us/rest/api/azure/devops/wit/queries/update?view=azure-devops-rest-7.2
#>
...
Assert-AdoEnv -Name 'Query' -Value $Query
...
#--- Call the API -----------------------------------------------------------
$uri = New-AdoUrl -Organization $Organization -Path "_apis/wit/queries/$Query" -ApiVersion $ApiVersion -Project $ProjectId
...
#--- Output -----------------------------------------------------------------
Write-Host "Queries ID: $($response.id)"
