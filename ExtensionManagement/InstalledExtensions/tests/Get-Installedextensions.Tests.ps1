...
<#
.SYNOPSIS
    Pester offline unit tests for Get-Installedextensions.ps1
...
BeforeAll {
    $ScriptPath = Join-Path $PSScriptRoot '..' 'Get-Installedextensions.ps1'
    $FixturePath = Join-Path $PSScriptRoot 'fixtures'
}

Describe 'Get-Installedextensions.ps1' -Tag 'Offline', 'ExtensionManagement' {
    ...
    Context 'Successful API call (mocked)' {
        ...
        BeforeAll {
            $fixture = Get-Content (Join-Path $FixturePath 'list_installed_extensions_200.json') -Raw | ConvertFrom-Json
        }
