...
# ============================================================================
# Offline unit tests for list_actions.sh
#
...
for arg in "\$@"; do
    if [[ "\$arg" == *"/_apis/audit"*"api-version="* ]]; then
        cat "${FIXTURES_DIR}/list_actions_200.json"
        exit 0
    fi
done
...
@test "list_actions.sh: exits with error when AZURE_DEVOPS_ORG is unset" {
    unset AZURE_DEVOPS_ORG
    run bash "$SCRIPT_DIR/list_actions.sh"
    [ "$status" -ne 0 ]
    [[ "$output" == *"AZURE_DEVOPS_ORG"* ]]
}

@test "list_actions.sh: exits with error when AZURE_DEVOPS_PAT is unset" {
...
@test "list_actions.sh: succeeds with valid env and stubbed curl" {
    run bash "$SCRIPT_DIR/list_actions.sh"
    [ "$status" -eq 0 ]
}

@test "list_actions.sh: sends Authorization header" {
    run bash "$SCRIPT_DIR/list_actions.sh"
    [ "$status" -eq 0 ]
