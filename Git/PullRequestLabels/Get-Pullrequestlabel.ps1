...
.DESCRIPTION
    Calls  GET {org}/_apis/git/repositories/{repository_id}/pullRequests/{pull_request_id}/labels/{label_id_or_name}?api-version=7.2
    Uses Basic Auth with a PAT stored in $env:AZURE_DEVOPS_PAT.

.LINK
    https://learn.microsoft.com/en-us/rest/api/azure/devops/git/pull-request-labels/get?view=azure-devops-rest-7.2
#>
...
param (
    ...
    [Parameter()]
    [string]$LabelIdOrName = $env:LABEL_ID_OR_NAME,
...
Assert-AdoEnv -Name 'LabelIdOrName' -Value $LabelIdOrName
...
#--- Call the API -----------------------------------------------------------
$uri = New-AdoUrl -Organization $Organization -Path "_apis/git/repositories/$RepositoryId/pullRequests/$PullRequestId/labels/$LabelIdOrName" -ApiVersion $ApiVersion -Project $ProjectId
...
if ($response.PSObject.Properties.Name -notcontains 'active') {
    Write-Warning "Unexpected response shape — missing 'active'. The server may not support api-version $ApiVersion."
}

#--- Output -----------------------------------------------------------------
Write-Host "Pull Request Labels: $($response.name)"
